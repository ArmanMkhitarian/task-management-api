...
Сначала всё обсудили с моделью, потом писали код. Только когда согласовали 
пошли в реализацию.

Правила вынес в CLAUDE.md. 
План по этапам в PLAN.md.

Делали по этапам, после каждого проверял и коммитил. 

Тесты делал в отдельной сессии, чтобы был свежий взгляд на готовый код. 


## Где правил предложения AI

**PATCH для статуса.** Модель хотела менять статус через общий PUT. Вынес в 
отдельный PATCH, его чаще всего дёргают, и полным PUT можно случайно перебить 
статус.

**IQueryable наружу.** Самое спорное место. Модель склонялась отдавать 
IQueryable из репозитория, так проще. Но тогда EF лезет в Application, а у нас 
так нельзя. Попросил её саму расписать минусы чистого варианта, глянул, для нас 
они мелкие, выбрал чистый.

**IGuidProvider.** Хотела завести интерфейс под генерацию Guid. Убрал, лишнее. 

**EF vs Dapper и N+1.** Сам поднял, модель не говорила. Она уточнила что без 
навигаций N+1 тут не возникает, согласился, договорились о проекции внутри 
Infrastructure.

**Нормализация email.** Решили нормализовать при записи. Заметил что тогда надо 
упростить сравнение из прошлого этапа, а то дублирование.

**Версии пакетов.** Стояли плавающие, попросил зафиксировать, чтобы у 
проверяющего собралось ровно то же.
