...
- **Нормализация email:** `AssigneeEmail` приводится к нижнему регистру (trim + lower, пустое → null) при записи — в `TaskItem.Create`/`UpdateDetails` через `EmailNormalizer` (Application). Значение фильтра нормализуется так же. Итог: в БД канонично, фильтр и сравнение «сменился ли исполнитель» — простым `==` (ordinal), без `ILIKE`/функционального индекса.
